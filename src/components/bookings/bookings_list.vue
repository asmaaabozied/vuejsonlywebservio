...
<!--    <BookingsLoader v-if='bookings.length === 0'/>-->
    <div v-if='bookings.length === 0' class="bg-white border-t border-gray-50 shadow-lg sm:rounded-lg">
      <div class='flex items-center p-4 border-b border-gray-100 sm:p-6 justify-between gap-x-2'>
        <div class='flex-1 text-sm font-medium text-gray-500'>
          ...
          <span class='text-second-color-200'>{{
              $t('Try adjusting your search or filter to find what you are looking for')
            }}</span>
    ...
    <div v-for='booking in bookings' :key='booking.id' class='bg-white border-t border-gray-50 shadow-lg sm:rounded-lg'>
      <div class='flex items-center p-4 border-b border-gray-100 sm:p-6 justify-between gap-x-2'>
        ...
        <span
            class='inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-200 text-gray-800'>{{
            this.$filters.transString(booking.booking_status?.status)
          }}</span>
        <span
            :class='[booking.payment ? "bg-green-200 text-green-800":"bg-gray-200 text-gray-800", "inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-gray-200 text-gray-800"]'>{{
            booking.payment ? $t('Paid') : $t('Not Paid')
          }}</span>
      ...
      <div class='p-4 sm:p-6 flex items-center sm:items-start space-x-4 rtl:space-x-reverse'>
        <div class='flex-shrink-0 w-20 h-20 bg-gray-200 rounded-lg overflow-hidden sm:w-20 sm:h-20'>
          <img :alt='booking.e_service?.name' :src='this.$filters.getFirstMediaThumb(booking.e_service)'
      ...
      <div class='px-4 sm:px-6 pb-4 sm:pb-6 sm:flex sm:justify-between'>
        <p class='text-sm font-medium text-gray-500'>
          {{ $t('Booking At') }}
          <time :datetime='this.$filters.formatDate(booking.booking_at)'>{{
              this.$filters.formatDate(booking.booking_at)
            }}
          </time>
        </p>
    ...
    <div v-if='bookings.length >10' class='flex w-full py-8'>
      <button
