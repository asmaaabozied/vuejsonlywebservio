...
<!--          <router-link v-if='!this.isAuth' :to='{name:"RegisterProvider"}'-->
<!--                       class='hidden px-4 py-2 text-sm text-white rounded-md border border-transparent shadow-sm md:inline-flex bg-main-color-600 hover:bg-main-color-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-main-color-500'-->
<!--                       type='button'>-->
<!--            {{ $t('Register AS Provider') }}-->
<!--          </router-link>-->

            <a  v-if='!this.isAuth' href="https://weservio.app/admin/public/eprovider/register" class="router-link-active router-link-exact-active hidden px-4 py-2 text-sm text-white rounded-md border border-transparent shadow-sm md:inline-flex bg-main-color-600 hover:bg-main-color-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-main-color-500" type="button" aria-current="page">Register AS Provider</a>
              ...
              <transition enter-active-class='transition duration-200 ease-out'
                ...
                <MenuItems
                  ...
                  <MenuItem v-slot='{ active }'>
                    ...
                    <router-link :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']" :to='{name:"password"}'>
                      {{ $t('Change Password') }}
                    </router-link>
                  </MenuItem>
                  <MenuItem v-slot='{ active }'>

                    <router-link :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']" :to='{name:"address"}'>
                      {{ $t('MyAddresses') }}
                    </router-link>
                  </MenuItem>
                  <MenuItem v-slot='{ active }'>
    ...
    <DisclosurePanel class='md:hidden'>
      ...
      <div class='pt-4 pb-3 border-t border-gray-200'>
        ...
        <div v-if='this.isAuth' class='mt-3 space-y-1'>
          ...
          <router-link
              :class="[($router.currentRoute.value.name === 'password') ? 'bg-main-color-50 border-main-color-500 text-main-color-700': 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700','text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4']"
              :to='{name:"password"}'>
            {{ $t('Change Password') }}
          </router-link>
          <router-link
              :class="[($router.currentRoute.value.name === 'address') ? 'bg-main-color-50 border-main-color-500 text-main-color-700': 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700','text-base font-medium sm:pl-5 sm:pr-6 block pl-3 pr-4 py-2 border-l-4']"
              :to='{name:"password"}'>
            {{ $t('MyAddresses') }}
          </router-link>
          <router-link
