    ...
    actions: {
           ...
           async addBookingAction({ commit, rootGetters }, booking) {
            ...
            // Declare a variable
             let e_provider_id;
             let user_id;
                e_provider_id =booking.e_provider.id;
        ...
        try {
            //console.log(booking);
            // Replace the following code with the appropriate code for your email service/API
            const emailServiceResponse = await sendEmailToUser(booking) // Replace with actual function call
            console.log(emailServiceResponse) // Log the response from the email service for debugging
        } catch (error) {
            ...
            }else  return { type: 'error', title: 'Error', message: response.data?.message }
        }
