...
export default {
    ...
    mutations: {
        ...
        UPDATE_STATUSES_Count(state, countstatuses) {
            state.countstatuses = countstatuses
        },
    ...
    actions: {
        async getStatusesAction({ commit ,getters}) {
            let queryParameters = {
            ...
            if (response.status === 200 && response.data?.success) {
                ...
                commit('UPDATE_STATUSES_Count', response.data.message)
